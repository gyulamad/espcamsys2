...
**Status:** §7 steps 1-5 implemented (see notes below). Steps 6-8 not yet implemented.
...
> **Implementation note (step 3, added when step 3 was implemented):** step 3 is monitoring-only AI (capture → downsample → inference → Serial log, no recording trigger) — see §7 step 3 below for exactly what was built and where. No contradictions with the plan as written or with steps 1/2's design; two judgment calls worth flagging: (1) the device-side `AI_ALARM_ENABLED_DEFAULT` §5.1 sketched, left open by the step 2 note above, is added here in `example.config.h` and now actually drives the device's own `aiEnabled` state, with the relay's `AiAlarmCommand.aiEnabled` (already parsed since step 1) applied to it at runtime — the two `AI_ALARM_ENABLED_DEFAULT`s (device-side boot default here, relay-side `aiAlarmEnabledDefault` from step 2) intentionally coexist, same relationship `config.h`'s other boot-time-default-vs-runtime-override values already have elsewhere in this codebase; (2) §6.1's "mutex... avoid two tasks touching the camera driver concurrently" is honored in spirit rather than literally — see §7 step 3's note for the actual (single-camera-owner, Core-1-only) design and why it still meets §6.1's real goal. Only §5.1's step-3-relevant config keys (`AI_ALARM_ENABLED_DEFAULT`, `AI_INFERENCE_INTERVAL_MS`, `AI_CONFIDENCE_THRESHOLD`) were added to `example.config.h` — the rest (`AI_CONFIRM_EXTRA_FRAMES`, `AI_CONFIRM_INTERVAL_MS`, `RECORD_RESOLUTION`, `RECORD_MERGE_WINDOW_MS`, `ROLLING_BUFFER_SECONDS`) stay for steps 4/5, so the example config doesn't grow keys nothing reads yet. **Real-hardware addendum:** after step 3 was actually flashed, two on-device fixes were needed before it would compile/run — (1) `ai_person_detect.h`'s tensor arena moved from a static 96KB array into PSRAM via `ps_malloc()` (the static array was a real memory problem on-device); (2) the vendored model's `person_detect_model_data.cc` has to sit next to the `.ino` at the sketch's top level, not inside `model/` — the Arduino build only auto-compiles `.cc`/`.cpp` files directly in the sketch folder, so a copy left inside `model/` silently isn't compiled in and the header-declared `g_person_detect_model_data` array is a link error. Both are reflected in the current `ai_person_detect.h`, `model/README.md`, and `.gitignore`.

> **Implementation note (step 4, added when step 4 was implemented):** step 4 is the §5.4 confirmation-burst debounce, still logging only — see §7 step 4 below for exactly what was built and where. No contradictions with the plan or with steps 1-3's design.

> **Implementation note (step 5, added when step 5 was implemented):** step 5 wires `CONFIRMED` into the existing recording pipeline and implements the §5.3 rolling pre-buffer — but **not** where §5.3 originally put it. See §7 step 5 below for the full reasoning: recording turned out to already happen entirely relay-side, off frames the camera is already continuously pushing, so the pre-buffer moved there too (full-resolution, not the originally-sketched device-side low-res RAM ring) rather than being built as first specified. This is a correction made per §8's own "read the existing recording code before implementing §5.3 — do not assume" instruction, not a contradiction of it.
...
3. **[DONE] Monitoring-only AI:** implement 96×96 grayscale capture + TFLite inference at `AI_INFERENCE_INTERVAL_MS`, log detections to Serial only — no recording trigger yet. Validate inference timing/accuracy in isolation. *(Firmware: `Arduino/sketch_sep12a_camera2_behind_NAT/logic.h` gained the hardware-free half of the pipeline — `shouldRunInference()` (inference-cadence gate), `downsampleRgb888ToGray()` (nearest-neighbor RGB888→96×96 grayscale downsample of the frame already captured for streaming, per §2's "prefer software-downscaling ... where possible" — no second camera call, no sensor reconfig), `quantizeGrayscaleToInt8()` (uint8→int8 for the model's input tensor), and `PersonDetectionResult evaluatePersonScores()` (raw two-score int8 model output → normalized 0..1 confidence via softmax + threshold decision). The new `Arduino/sketch_sep12a_camera2_behind_NAT/ai_person_detect.h` wraps the actual TFLite Micro model/interpreter (kept out of `logic.h` per that file's own hardware-free rule) — see its header comment for the **library choice this step records per §3/§8's "pick and pin" instruction: `espressif/esp-tflite-micro` v1.3.3**, and `model/README.md` for how to fetch the (not vendored — large generated binary) reference person-detection model into `model/person_detect_model_data.h`; the sketch compiles and runs fine without it, AI monitoring is just reported unavailable. The `.ino` wires it together per §6.1's Core 0/Core 1 split, but not via §6.1's literal "mutex-protected concurrent camera access" — instead `loop()` (Core 1) decodes+downsamples the frame it already captured for streaming (gated by `shouldRunInference()`), handing a 96×96 grayscale sample to the new `aiTask()` (Core 0, `xTaskCreatePinnedToCore`) through a short critical section (`aiFrameBuf`/`aiFrameFresh`/`aiFrameMux`); only `aiTask()` ever calls the (slow, ~200-400ms) `Invoke()`, so the push loop is never stalled by it, and the camera driver itself is only ever touched from Core 1 — achieving §6.1's actual goal (don't let inference stall uploads, don't let two tasks race the camera driver) without the literal frame hand-off/mutex shape §6.2's pseudocode sketches. This step also applies `AI_ALARM_ENABLED_DEFAULT` as the device's own boot-time AI on/off state for the first time (`bool aiEnabled` in the `.ino`) and makes the already-parsed `AiAlarmCommand.aiEnabled` from the relay actually take effect instead of only being logged (step 1/2 note on §5.6) — resolving the "still open for whichever of steps 3+ first gives the device its own AI state machine" item from the step 2 note at the top of this document. Config: `example.config.h` gained `AI_ALARM_ENABLED_DEFAULT`, `AI_INFERENCE_INTERVAL_MS`, `AI_CONFIDENCE_THRESHOLD` (§5.1's other keys — `AI_CONFIRM_EXTRA_FRAMES`, `AI_CONFIRM_INTERVAL_MS`, `RECORD_RESOLUTION`, `RECORD_MERGE_WINDOW_MS`, `ROLLING_BUFFER_SECONDS` — are steps 4/5's concern, not added yet), as `const`/`const bool`/etc. rather than `#define`, matching this file's existing style (same convention-over-original-sketch reasoning as steps 1/2). Tests: `tests/cpp/test_alarm_logic.cpp` gained 20 new assertions covering all four new `logic.h` functions above (cadence gate, downsample correctness/coverage, quantization boundaries, softmax+threshold semantics including the >= boundary) — everything that doesn't require a real TFLite interpreter/model; the model wrapper itself (`ai_person_detect.h`) is intentionally not unit-tested, same reasoning `logic.h`'s header comment already gives for keeping hardware out of that file. All pre-existing tests still pass unchanged — nothing about this step's design conflicts with what steps 1/2 built (the command channel's `aiEnabled` field now has somewhere to go, rather than needing to change shape). "Validate inference timing/accuracy in isolation" (this step's own closing instruction) still needs real hardware + the vendored model in place and is a field-testing task, not something resolvable in this environment — flagged here rather than silently skipped.)*
4. **[DONE] Confirmation burst:** add the extra-frame debounce logic (§5.4), still logging only. *(Firmware: `Arduino/sketch_sep12a_camera2_behind_NAT/logic.h` gained the §5.2 MONITORING/CONFIRMING half of the state machine as pure, hardware-free logic — `AiAlarmMode` (`MONITORING`/`CONFIRMING`; `RECORDING`/`DEBOUNCE`/`LIVE_PEEK` aren't needed until steps 5-7 give them something to actually mean), `ConfirmationBurstState`, and `updateConfirmationBurst()` (feeds one inference's `personDetected` bit through the transitions, implementing §5.4's "simplest: all extra frames must also be positive" rule — a single non-positive frame anywhere in the burst rejects it immediately), plus `currentAiSampleIntervalMs()` (picks `AI_INFERENCE_INTERVAL_MS` vs the faster `AI_CONFIRM_INTERVAL_MS` based on the current mode). The `.ino`'s `aiTask()` (Core 0) now runs every inference result through `updateConfirmationBurst()` and logs `ENTERED_CONFIRMING`/`CONFIRMED`/`REJECTED` distinctly (still only logs — no recording trigger, per this step's own instruction); the resulting mode is published to a new `currentAiMode` (read by `aiSampleFromFrame()` on Core 1) so the *capture* cadence itself speeds up to `AI_CONFIRM_INTERVAL_MS` during a burst, which is what actually delivers §5.4's "not waiting for the normal monitoring interval for the extra confirmation frames" — adapted from §6.2's pseudocode (a synchronous blocking `runConfirmationBurst()` helper) to this codebase's actual async Core0/Core1 cadence-gated handoff from step 3, the same kind of literal-pseudocode-vs-actual-architecture adaptation as step 3's Core-ownership note above; no contradiction with §5.4's rule itself, only with the pseudocode's specific (illustrative, not final) shape. A burst in progress when AI monitoring is toggled off mid-burst is abandoned (state reset to `MONITORING`) rather than resumed stale once it's re-enabled — a judgment call the plan didn't specify, chosen because resuming a burst against frames from a potentially different moment/scene seemed more likely to misfire than just starting fresh. Config: `example.config.h` gained `AI_CONFIRM_EXTRA_FRAMES` (2) and `AI_CONFIRM_INTERVAL_MS` (150), the two §5.1 keys step 3's note left for this step. Tests: `tests/cpp/test_alarm_logic.cpp` gained 10 new assertions on `updateConfirmationBurst()` (single-step transitions in both directions, a full multi-frame burst sequence, immediate rejection on the very first confirmation frame, a fresh trigger correctly accepted after a prior rejection, the `confirmExtraFrames <= 0` edge case) and `currentAiSampleIntervalMs()`. All pre-existing tests (including step 3's) still pass unchanged — this step only adds new functions and a new caller of `shouldRunInference()` with a variable interval; `shouldRunInference()`'s own signature/semantics are untouched. Same as step 3, actually observing confirmed-vs-rejected behavior against real scenes needs the vendored model on real hardware and is field-tuning, not something resolvable here.)*
5. **[DONE] Recording trigger + rolling buffer:** wire confirmed detections into the existing recording pipeline, prepend the rolling pre-buffer (§5.3). *(**Deviation from §5.3 as written, recorded per §8's "read the existing recording code before implementing §5.3 — do not assume":** §5.3 originally sketched the pre-buffer as a device-side RAM ring of 96×96 grayscale monitoring frames. Reading `server.js`'s actual recording pipeline first (as §8 instructed) shows recording already happens entirely relay-side, built from whatever full-resolution JPEG frames the camera is already continuously pushing for live view (`cam.emitter.emit('frame', ...)` fires on every push, recording or not — see `startRecording()`'s own comment). So the pre-buffer now lives there instead: `nodejs/camera-relay/lib/frameBuffer.js` (new) is a pure `pushAndTrim(buffer, frame, nowMs, windowSeconds)` — a rolling time window, not a fixed slot count — wired into both frame-arrival points (`/upload/:id` and the push-socket handler) in `server.js`, into a new `cam.frameBuffer` per camera. `startRecording()` now seeds a fresh clip with whatever's in `cam.frameBuffer` (written out first, in arrival order, before the live `cam.emitter` listener continues the same frame numbering) — `rec.frameCount`/`firstFrameAt`/`lastFrameAt` include the pre-roll frames too, so `computeFps()` (untouched, step-3-era `video.js`) measures fps across the *whole* clip including pre-roll, not just the post-trigger portion. This benefits every recording trigger equally (AI alarm, GPIO alarm, manual dashboard RECORD button all funnel through the same `startRecording()`), not just AI-alarm-initiated ones — a natural consequence of where the mechanism actually lives, not scope creep: there was no way to make it AI-alarm-specific without threading a flag through the entire push/record path for no real benefit. This is a genuinely *better* realization of §5.3's actual intent (full-resolution pre-roll, not a low-res tradeoff) using frames that were already arriving at zero additional device cost or bandwidth — see `frameBuffer.js`'s own header comment for the full reasoning, including why §5.3's explicit "do not buffer high-res frames" warning doesn't apply here (that warning was about the *device's* limited RAM, not the relay's). No contradiction with any existing test or behavior; recording without a full pre-buffer window still works exactly as before (an empty/short `cam.frameBuffer` just means less or no pre-roll, never an error). Config: relay's `example.config.js` gained `rollingPreBufferSeconds` (default 3, matching §5.1's suggested `ROLLING_BUFFER_SECONDS`), plus a `ROLLING_PRE_BUFFER_SECONDS` env-var override in `server.js` (same precedent as `PORT`/`PUSH_PORT`/`CAM_KEY`) purely so the new e2e test can pick a deterministic value without touching a real `config.js`. Firmware side: the actual trigger is far simpler than the pre-buffer, precisely because §1 already asked for pipeline reuse — `logic.h` gained `shouldTriggerAiAlarmRecording()` (a named, tested predicate: true only on `CONFIRMED`), and `aiTask()` (Core 0) now sets a new `volatile bool aiAlarmTriggerRequested` flag on that outcome instead of calling `triggerAlarmRecording()` directly — a new `checkAiAlarmTrigger()`, polled from `loop()` (Core 1) right next to the existing GPIO `checkAlarmTrigger()`, does the actual (blocking, `HTTPClient`-based) network call, so Core 0 never blocks waiting on WiFi/HTTP and the existing "networking stays on Core 1" pattern from step 3/4 holds. `triggerAlarmRecording()`/`sendRecordRequest()`/`ALARM_RECORD_SECONDS`/`ALARM_RECORD_ALL_CAMERAS` are reused completely unchanged — no new device-side config constants were needed for the trigger itself. Because `sendRecordRequest()` already start-or-extends (documented in its own comment), a continuously-visible person naturally keeps a single recording alive across repeated `CONFIRMED` outcomes without any new "am I already recording" tracking on the device — §5.5's merge-window/`DEBOUNCE` handling (precise grace-period semantics distinct from a recording's own timeout, plus the dashboard's `DEBOUNCE` label) is still genuinely step 6's job, not accidentally solved here. Tests: `tests/node/test_frameBuffer.js` (new, 6 assertions) covers `pushAndTrim()` in isolation — window trimming, ordering, a long-gap-drops-everything case, a zero-window edge case; `tests/cpp/test_alarm_logic.cpp` gained 2 assertions on `shouldTriggerAiAlarmRecording()`. `tests/e2e/test_recording_prebuffer_e2e.js` (new) is the one that actually proves the wiring works, not just the pieces in isolation: it spawns the real relay and real `ffmpeg`, pushes real JPEG frames steadily *before* calling `POST /record/:id`, then a few more during the recording window, and uses `ffprobe` on the resulting `.mp4` to confirm it contains meaningfully more frames than just the post-trigger pushes — i.e. the pre-roll frames actually made it into the encoded clip, not just into `cam.frameBuffer`. `run_tests.sh`'s e2e section was changed from hardcoding the single existing e2e file to globbing `tests/e2e/test_*.js` (matching the pattern the Node/PHP sections already used), so this and any future e2e file run automatically — the existing `test_ai_alarm_e2e.js` behavior is unchanged, just now discovered instead of named explicitly. All 224 assertions across every suite pass, including both e2e tests together (different ports, run sequentially, no collision). "Wire confirmed detections into the existing recording pipeline" is otherwise complete; field-verifying real false-positive/negative rates and clip quality on actual hardware is step 8's job.)*
...
- ~~Exact TFLite Micro library/version to depend on (Arduino Library Manager availability, or vendoring Espressif's `tflite-micro-esp-examples`) — needs to be picked and the choice recorded.~~ **[DONE] Picked in step 3:** `espressif/esp-tflite-micro` v1.3.3 (the actively maintained successor to the `tflite-micro-esp-examples` named here) — see `Arduino/sketch_sep12a_camera2_behind_NAT/ai_person_detect.h`'s header comment and §7 step 3's note for the reasoning and install steps. The model itself is still not vendored in-repo (see that file and `model/README.md`) — only the library/version choice was the open item here.
